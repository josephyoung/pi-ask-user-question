...
type Scenario = {
  args: unknown;
  toolCalls?: unknown[];
  nextTurnArgs?: unknown;
  retryArgs?: unknown;
  marker?: string;
  keys?: string;
  followupMarker?: string;
  followupKeys?: string;
  steps?: Array<{ marker: string; keys: string; delayMs?: number; exit?: boolean }>;
  expected?: string;
  abort?: boolean;
  fieldAssistResponses?: string[];
  fieldAssistDelayMs?: number;
  timeoutMs?: number;
  manualExit?: boolean;
...
function serveChatCompletion(request: import("node:http").IncomingMessage, response: import("node:http").ServerResponse): boolean {
  if (request.url === "/v1/chat/completions" && request.method === "POST") {
    ...
    request.on("end", () => {
      if (raw.includes("Recovered Submitted Form confirmation completed")) {
        completion(response, { role: "assistant", content: "RECOVERY_CONTINUED" }, "stop");
        return;
      }
      const payload = JSON.parse(raw) as { messages?: Array<{ role?: string; content?: string }>; tools?: unknown[] };
      const messages = payload.messages ?? [];
      const latest = messages.at(-1);
      if (!payload.tools?.length) {
        fieldAssistRequests += 1;
        const content = scenario?.fieldAssistResponses?.shift() ?? "Generated by isolated Field Assist";
        const send = () => completion(response, { role: "assistant", content }, "stop");
        if (scenario?.fieldAssistDelayMs) setTimeout(send, scenario.fieldAssistDelayMs);
        else send();
        return;
      }
      if (latest?.role === "tool") {
        ...
        if (scenario?.retryArgs !== undefined && structuredRetries === 0) {
          structuredRetries += 1;
          toolBatchIndex += 1;
          completion(response, { role: "assistant", content: null, tool_calls: [{
            index: 0,
            id: `call_acceptance_${toolBatchIndex}_0`,
            type: "function",
            function: { name: "ask_user_question", arguments: JSON.stringify(scenario.retryArgs) },
          }] }, "tool_calls");
          return;
        }
...
async function runPi(next: Scenario): Promise<string> {
  scenario = next; lastToolResult = ""; lastToolResults = []; toolResultBatches = []; lastToolDetails = undefined; remoteAttempts = 0; toolBatchIndex = 0; fieldAssistRequests = 0; structuredRetries = 0; requestLog.length = 0; remoteSeen.length = 0;
  const args = [
    ...(next.sessionId ? ["--session-id", next.sessionId] : []),
    "--no-skills", "--no-context-files", "--approve", "--model", "acceptance/local", "--api-key", "test",
    "--thinking", "off", "--tools", "ask_user_question", "RUN ACCEPTANCE SCENARIO",
  ];
  const env = acceptanceEnv(next.env);
  ...
  let output = ""; let stepIndex = 0; let stepOffset = 0; let exiting = false; let exitTimer: ReturnType<typeof setTimeout> | undefined;
  const requestExit = () => {
    ...
    const attempt = () => {
      child.write("\u0004");
      attempts += 1;
      if (attempts < 20) exitTimer = setTimeout(attempt, 100);
    };
    exitTimer = setTimeout(attempt, 25);
  };
  const receive = (data: Buffer) => {
    ...
    if (step && newVisible.includes(step.marker)) {
      stepIndex += 1;
      stepOffset = output.length;
      const act = () => step.exit ? requestExit() : child.write(step.keys);
      if (step.delayMs) setTimeout(act, step.delayMs);
      else act();
    }
    if (!next.manualExit && !exiting && (visible.includes("CONTINUED:") || (next.abort && !next.nextTurnArgs && visible.includes("Question was aborted")))) requestExit();
  };
  child.onData(data => receive(Buffer.from(data)));
  const status = await new Promise<number | null>((resolvePromise, reject) => {
    const timer = setTimeout(() => { child.kill(); reject(new Error(`real pi timeout steps=${stepIndex}/${steps.length} requests=${requestLog.join(",")} tool=${lastToolResult}:\n${clean(output).slice(0, 3000)}\n---tail---\n${clean(output).slice(-3000)}`)); }, next.timeoutMs ?? 30_000);
    child.onExit(({ exitCode }) => { clearTimeout(timer); if (exitTimer) clearTimeout(exitTimer); resolvePromise(exitCode); });
  });
  ptys.delete(child);
...
function acceptanceEnv(overrides: Record<string, string | undefined> = {}): Record<string, string> {
  return Object.fromEntries(Object.entries({
    ...process.env,
    PI_CODING_AGENT_DIR: state,
    NPM_CONFIG_CACHE: join(sandbox, "npm-cache"),
    TEST_REMOTE_AUTH: "Bearer acceptance-secret",
    TEST_REMOTE_SESSION: "acceptance-cookie",
    TERM: "xterm-256color",
    ...overrides,
...
beforeAll(async () => {
  ...
  if (packageSource.endsWith(".tgz")) {
    ...
    mkdirSync(artifact);
    run("tar", ["-xzf", packageSource, "-C", artifact, "--strip-components=1"], sandbox);
    await runAsync("npm", ["install", "--omit=dev", "--legacy-peer-deps", "--package-lock=false", "--prefix", artifact], sandbox, {
      NPM_CONFIG_CACHE: join(sandbox, "npm-cache"),
    });
    installSource = artifact;
  }
  const lockedPi = realpathSync(piExecutable);
  const installOutput = await runAsync(process.execPath, [lockedPi, "install", installSource, "-l"], project, { PI_CODING_AGENT_DIR: state, NPM_CONFIG_CACHE: join(sandbox, "npm-cache") });
  expect(installOutput).toContain("Installed");
  expect(run(process.execPath, [lockedPi, "--version"], project, { PI_CODING_AGENT_DIR: state }).trim()).toBe("0.82.1");
  ...
  writeFileSync(join(extensions, "stale-revision-fixture.ts"), `export default function (pi) {
    let injected = false;
    pi.on("tool_execution_start", (event, ctx) => {
      if (process.env.PI_AUQ_INJECT_STALE !== "1" || injected || event.toolName !== "ask_user_question" || !event.args?.confirm) return;
      setTimeout(() => {
        const current = [...ctx.sessionManager.getBranch()].reverse().find(entry => entry.type === "custom" && entry.customType === "ask-user-question.form-interaction.v1");
        if (!current?.data || current.data.state !== "awaiting_confirmation") return;
        injected = true;
        pi.appendEntry("ask-user-question.form-interaction.v1", { ...current.data, revision: current.data.revision + 1 });
      }, 100);
    });
  }`);
  writeFileSync(join(state, "ask-user-question.auth.json"), JSON.stringify({
    version: 1,
    rules: [{
      origin: baseUrl,
      pathPrefix: "/remote-post",
      headers: { Authorization: "TEST_REMOTE_AUTH" },
      cookies: { session: "TEST_REMOTE_SESSION" },
    }],
  }));
}, 120_000);
...
describe("real Pi CLI acceptance in isolated Git-installed environment", () => {
  it("prepares compatibility input before TypeBox and retries one structured invalid result", async () => {
    await runPi({
      args: { questions: JSON.stringify([{ question: "Broken compatibility", options: "[]" }]) },
      retryArgs: { question: "Prepared compatibility", options: '["A","B"]', default: "A" },
      marker: "Prepared compatibility",
      keys: "\r",
      expected: '"A"',
    });
    expect(toolResultBatches[0]?.[0]).toContain('"status":"invalid"');
  ...
  it("confirms a Submitted Form and completes a return-to-modify revision", async () => {
    const confirmed = await runPi({
      args: { title: "Leave request", questions: [{ id: "reason", question: "Reason", default: "Personal", required: true }] },
      nextTurnArgs: { confirm: true, formIds: ["call_acceptance_1_0"] },
      steps: [
        { marker: "Reason: Personal", keys: "\u0013" },
        { marker: "FIRST_DONE", keys: "CONFIRM FORM\r" },
        { marker: "Submitted Forms · final confirmation", keys: "\r" },
      ],
      expected: '"reason":"Personal"',
    });
    expect(lastToolDetails).toMatchObject({
      status: "confirmed",
      confirmationOfToolCallId: "call_acceptance_1_0",
      forms: [{ formId: "call_acceptance_1_0", answer: { reason: "Personal" } }],
    });
    expect(confirmed).toContain("Leave request");

    await runPi({
      args: { title: "Revision request", questions: [{ id: "reason", question: "Revision reason", default: "Old", required: true }] },
      nextTurnArgs: { confirm: true, formIds: ["call_acceptance_1_0"] },
      steps: [
        { marker: "Revision reason: Old", keys: "\u0013" },
        { marker: "FIRST_DONE", keys: "REVISE FORM\r" },
        { marker: "Submitted Forms · final confirmation", keys: "\u001b[B\r" },
        { marker: "Question 1/1 · Tab/Shift+Tab navigate · Ctrl+S submit", keys: "\u0015New\u0013" },
        { marker: "Submitted Forms · final confirmation", keys: "\r" },
      ],
      expected: '"reason":"New"',
    });
    expect(lastToolDetails).toMatchObject({ status: "confirmed", forms: [{ answer: { reason: "New" } }] });

    await runPi({
      args: { title: "Cancel revision", questions: [{ id: "reason", question: "Cancel revision reason", default: "Canonical", required: true }] },
      nextTurnArgs: { confirm: true, formIds: ["call_acceptance_1_0"] },
      steps: [
        { marker: "Cancel revision reason: Canonical", keys: "\u0013" },
        { marker: "FIRST_DONE", keys: "CANCEL REVISION\r" },
        { marker: "Submitted Forms · final confirmation", keys: "\u001b[B\r" },
        { marker: "Question 1/1 · Tab/Shift+Tab navigate · Ctrl+S submit", keys: "\u0015Discarded\u001b" },
        { marker: "Submitted Forms · final confirmation", keys: "\r" },
      ],
      expected: '"reason":"Canonical"',
    });
    expect(lastToolDetails).toMatchObject({ status: "confirmed", forms: [{ answer: { reason: "Canonical" } }] });
  }, 120_000);

  it("refreshes a stale confirmation revision in place through real Pi", async () => {
    await runPi({
      args: { title: "Stale request", questions: [{ id: "reason", question: "Stale reason", default: "Canonical", required: true }] },
      nextTurnArgs: { confirm: true, formIds: ["call_acceptance_1_0"] },
      env: { PI_AUQ_INJECT_STALE: "1" },
      steps: [
        { marker: "Stale reason: Canonical", keys: "\u0013" },
        { marker: "FIRST_DONE", keys: "CONFIRM STALE\r" },
        { marker: "Submitted Forms · final confirmation", keys: "\r", delayMs: 250 },
        { marker: "Submitted Forms · final confirmation", keys: "\r" },
      ],
      expected: '"reason":"Canonical"',
    });
  ...
  it("renders a terminal confirmation read-only through the real Pi transcript", async () => {
    const sessionId = "ask-user-question-terminal-render-e2e";
    await runPi({
      args: { title: "Terminal request", questions: [{ id: "reason", question: "Terminal reason", default: "Read only", required: true }] },
      nextTurnArgs: { confirm: true, formIds: ["call_acceptance_1_0"] },
      sessionId,
      steps: [
        { marker: "Terminal reason: Read only", keys: "\u0013" },
        { marker: "FIRST_DONE", keys: "CONFIRM TERMINAL\r" },
        { marker: "Submitted Forms · final confirmation", keys: "\r" },
      ],
      expected: '"reason":"Read only"',
    });

    const resumed = spawnPty(process.execPath, [realpathSync(piExecutable),
      "--session-id", sessionId, "--no-skills", "--no-context-files", "--approve",
      "--model", "acceptance/local", "--api-key", "test", "--thinking", "off", "--tools", "ask_user_question",
    ], { cwd: project, env: acceptanceEnv(), cols: 100, rows: 32 });
    ptys.add(resumed);
    let output = "";
    let expanded = false;
    let exiting = false;
    ...
    resumed.onData(data => {
      output += data;
      const visible = clean(output);
      if (!expanded && visible.includes("Press ctrl+o")) {
        expanded = true;
        setTimeout(() => resumed.write("\u000f"), 100);
      }
      if (visible.includes("Form confirmation · confirmed") && visible.includes("Terminal request")) exit();
    });
    const exitCode = await new Promise<number>((resolvePromise, reject) => {
      const timer = setTimeout(() => { resumed.kill(); reject(new Error(`terminal renderer resume timed out:\n${clean(output).slice(-5000)}`)); }, 20_000);
      resumed.onExit(({ exitCode }) => {
        clearTimeout(timer);
    ...
    expect(clean(output)).toContain("Form confirmation · confirmed");
    expect(clean(output)).toContain("Terminal request");
    expect(clean(output)).toContain("Terminal reason: Read only");
    expect(clean(output)).not.toContain("RECOVERY_CONTINUED");
    expect(clean(output)).not.toContain("Unable to restore the pending question interaction");
  }, 40_000);

  it("runs Generate, Polish, semantic retry, abort, and credential rejection in isolated Field Assist sessions", async () => {
    const sessionCountBefore = sessionFiles(join(state, "sessions")).length;
    const output = await runPi({
      args: { question: "Assisted text", inputType: "textarea", default: "Original", required: true },
      steps: [
        { marker: "Field Assist: Ctrl+G generate", keys: "\u0007" },
        { marker: "Generated by isolated Field Assist", keys: "\u0013" },
      ],
      expected: '"Generated by isolated Field Assist"',
    });
    expect(lastToolDetails).toEqual({ status: "answered", answer: "Generated by isolated Field Assist" });
    expect(output).toContain("CONTINUED:");
    expect(fieldAssistRequests).toBe(1);

    await runPi({
      args: { question: "Polish text", inputType: "textarea", default: "rough text", required: true },
      fieldAssistResponses: ["Polished sentence with clearer wording"],
      steps: [
        { marker: "Field Assist: Ctrl+G generate", keys: "\u0010" },
        { marker: "Polished sentence with clearer wording", keys: "\u0013" },
      ],
      expected: '"Polished sentence with clearer wording"',
    });
    expect(fieldAssistRequests).toBe(1);

    const parentAbort = await runPi({
      args: { question: "Parent abort assisted", inputType: "textarea", default: "Keep parent value", required: true },
      fieldAssistResponses: ["Late parent output"], fieldAssistDelayMs: 2_000, abort: true,
      steps: [
        { marker: "Field Assist: Ctrl+G generate", keys: "\u0007" },
        { marker: "AI generating", keys: "\u0003", delayMs: 250 },
      ],
    });
    expect(parentAbort).toContain("Question was aborted");
    expect(fieldAssistRequests).toBe(1);

    await runPi({
      args: { question: "Retry assisted", inputType: "textarea", default: "Original", required: true },
      fieldAssistResponses: ["ask_user_question RETRY_ONLY_CHILD_PROMPT", "Alternative generated content"],
      steps: [
        { marker: "Field Assist: Ctrl+G generate", keys: "\u0007" },
        { marker: "Alternative generated content", keys: "\u0013" },
      ],
      expected: '"Alternative generated content"',
    });
    expect(fieldAssistRequests).toBe(2);
    expect(sessionFiles(join(state, "sessions")).length - sessionCountBefore).toBe(4);
    const persistedMainSessions = sessionFiles(join(state, "sessions")).map(file => readFileSync(file, "utf8")).join("\n");
    expect(persistedMainSessions).not.toContain("RETRY_ONLY_CHILD_PROMPT");

    await runPi({
      args: { question: "Abort assisted", inputType: "textarea", default: "Keep original", required: true },
      fieldAssistResponses: ["Late child output"], fieldAssistDelayMs: 2_000,
      steps: [
        { marker: "Field Assist: Ctrl+G generate", keys: "\u0007" },
        { marker: "AI generating", keys: "\u001b", delayMs: 250 },
        { marker: "AI assist was aborted", keys: "\u0013" },
      ],
    ...
    await runPi({
      args: { question: "Credential assisted", inputType: "textarea", default: "token=abcdefghijklmnopqrstuvwxyz123456", required: true },
      steps: [
        { marker: "Field Assist: Ctrl+G generate", keys: "\u0007" },
        { marker: "obvious secret credentials", keys: "\u001b" },
      ],
      expected: "cancelled",
    });
    expect(fieldAssistRequests).toBe(0);

  }, 120_000);

  it("bounds a failing current-model child by timeout and keeps the previous value", async () => {
    await runPi({
      args: { question: "Timeout assisted", inputType: "textarea", default: "Keep timeout value", required: true },
      fieldAssistResponses: ["Too late"], fieldAssistDelayMs: 65_000, timeoutMs: 75_000,
      steps: [
        { marker: "Field Assist: Ctrl+G generate", keys: "\u0007" },
        { marker: "AI assist timed out", keys: "\u0013" },
      ],
      expected: '"Keep timeout value"',
    });
    expect(fieldAssistRequests).toBe(1);
  }, 90_000);

  it("rejects a CLI-key-only current model before a Field Assist child request", async () => {
    const modelsPath = join(state, "models.json");
    const persisted = readFileSync(modelsPath, "utf8");
    const withoutPersistedKey = JSON.parse(persisted) as { providers: { acceptance: { apiKey?: string } } };
    delete withoutPersistedKey.providers.acceptance.apiKey;
    writeFileSync(modelsPath, JSON.stringify(withoutPersistedKey));
    try {
      await runPi({
        args: { question: "CLI-only assisted", inputType: "textarea", default: "Keep CLI value", required: true },
        steps: [
          { marker: "Field Assist: Ctrl+G generate", keys: "\u0007" },
          { marker: "no reusable persisted authentication", keys: "\u0013" },
        ],
        expected: '"Keep CLI value"',
      });
      expect(fieldAssistRequests).toBe(0);
    } finally {
      writeFileSync(modelsPath, persisted);
    }
  });

  it("recovers an open confirmation across a real Pi process boundary", async () => {
    scenario = {
      args: { title: "Recovery form", questions: [{ id: "reason", question: "Recovery reason", default: "Persist me", required: true }] },
      nextTurnArgs: { confirm: true, formIds: ["call_acceptance_1_0"] },
    };
    lastToolResult = ""; lastToolResults = []; toolResultBatches = []; toolBatchIndex = 0; requestLog.length = 0;
    const baseArgs = [
      "--session-id", "ask-user-question-recovery-e2e", "--no-skills", "--no-context-files", "--approve",
      "--model", "acceptance/local", "--api-key", "test", "--thinking", "off", "--tools", "ask_user_question",
    ];
    const first = spawnPty(process.execPath, [realpathSync(piExecutable), ...baseArgs, "START RECOVERY"], {
      cwd: project, env: acceptanceEnv(), cols: 100, rows: 32,
    });
    ptys.add(first);
    let firstOutput = "";
    ...
    first.onData(data => {
      firstOutput += data;
      const visible = clean(firstOutput);
      if (firstStep === 0 && visible.includes("Recovery reason: Persist me")) { firstStep = 1; first.write("\u0013"); }
      else if (firstStep === 1 && visible.includes("FIRST_DONE")) { firstStep = 2; first.write("CONFIRM RECOVERY\r"); }
      else if (firstStep === 2 && visible.includes("Submitted Forms · final confirmation")) {
        firstStep = 3;
        setTimeout(() => first.kill(), 50);
      }
    });
    await new Promise<void>((resolvePromise, reject) => {
      const timer = setTimeout(() => { first.kill(); reject(new Error(`first recovery process timed out:\n${clean(firstOutput).slice(-4000)}`)); }, 30_000);
      first.onExit(() => { clearTimeout(timer); resolvePromise(); });
    });
    ptys.delete(first);
    expect(firstStep).toBe(3);

    const second = spawnPty(process.execPath, [realpathSync(piExecutable), ...baseArgs], {
      cwd: project, env: acceptanceEnv(), cols: 100, rows: 32,
    });
    ptys.add(second);
    let secondOutput = "";
    ...
    second.onData(data => {
      secondOutput += data;
      const visible = clean(secondOutput);
      if (!confirmed && visible.includes("Submitted Forms · final confirmation")) {
        confirmed = true;
        setTimeout(() => second.write("\r"), 500);
      }
      if (visible.includes("RECOVERY_CONTINUED")) requestSecondExit();
    });
    const exitCode = await new Promise<number>((resolvePromise, reject) => {
      const timer = setTimeout(() => { second.kill(); reject(new Error(`resumed recovery process timed out:\n${clean(secondOutput).slice(-5000)}`)); }, 30_000);
      second.onExit(({ exitCode }) => {
        clearTimeout(timer);
        if (exitTimer) clearTimeout(exitTimer);
        resolvePromise(exitCode);
      });
    ...
    expect(clean(secondOutput)).toContain("RECOVERY_CONTINUED");
    const continuationCount = sessionFiles(join(state, "sessions")).flatMap(file => readFileSync(file, "utf8").split("\n"))
      .flatMap(line => line ? [JSON.parse(line) as { type?: string; customType?: string }] : [])
      .filter(entry => entry.type === "custom_message" && entry.customType === "ask-user-question.recovered-continuation.v1").length;
  ...
  it("normalizes every compatible structured default into visible canonical form state", async () => {
    ...
    expect(lastToolDetails).toMatchObject({ status: "answered", formId: "call_acceptance_1_0", answer: { label: 2, object: "b", typed: "0", numeric: 0, multiple: ["ts", "py"] } });
  ...
  it("clears optional select and treeSelect answers and omits them from grouped results", async () => {
    ...
    expect(lastToolDetails).toMatchObject({ status: "answered", formId: "call_acceptance_1_0", answer: {} });
  ...
  it("returns missing-base guidance before UI, keeps remote errors field-local, supports cancel and abort", async () => {
    ...
    expect(lastToolDetails).toMatchObject({ status: "answered", formId: "call_acceptance_1_0", answer: { confirm: true, reason: "kept" } });
  ...
  it("drives authenticated POST, search, appended pagination and every field-local retry through real Pi", async () => {
    const mappedSource = { type: "api", endpoint: `${baseUrl}/remote-post`, method: "POST", params: { fixed: "yes" }, resultPath: "payload.rows", totalPath: "payload.total", idField: "code", labelField: "text", childrenField: "nodes", extraFields: ["meta"] };
    ...
    expect(remoteSeen[0]).toMatchObject({ method: "POST", headers: { authorization: "Bearer acceptance-secret", cookie: "session=acceptance-cookie" }, body: '{"fixed":"yes"}' });
    ...
    expect(remoteSeen.every(item => item.headers.authorization === undefined && item.headers.cookie === undefined)).toBe(true);
  ...
  it("executes multiple calls from one model response sequentially without overlapping cards", async () => {
    const output = await runPi({
      ...
      steps: [
        ...
        { marker: "Duplicate pending", keys: "\u0013" },
      ],
      expected: '"second":"retry"',
    });
    expect(toolResultBatches[0]?.some(result => result.includes('"first":"kept"'))).toBe(true);
    expect(toolResultBatches[0]?.some(result => result.includes('"second":"retry"'))).toBe(true);
    expect(output).toContain("Original pending: kept");
    expect(output).toContain("Duplicate pending: retry");
  }, 120_000);
  ...
  it("releases cancelled form state and opens a fresh question in the next real Pi Agent turn", async () => {
    ...
    expect(lastToolDetails).toMatchObject({ status: "answered", answer: { second: "fresh" } });
  ...
  it("aborts an unassisted textarea surface and opens a fresh question in the next Agent turn", async () => {
    const output = await runPi({
      args: { question: "Abort textarea", inputType: "textarea", fieldAssist: false, default: "draft" },
      nextTurnArgs: { question: "After textarea abort", default: "fresh" },
      steps: [
        { marker: "Enter submit · Ctrl+S submit · Esc cancel", keys: "\u0003" },
        { marker: "FIRST_DONE", keys: "RUN SECOND TURN\r" },
        { marker: "After textarea abort", keys: "fresh\r", delayMs: 250 },
      ],
      expected: '"fresh"',
      abort: true,
    });
    expect(toolResultBatches[0]?.some(result => result.includes("Question was aborted"))).toBe(true);
    expect(output).toContain("After textarea abort: fresh");
  }, 120_000);
});
