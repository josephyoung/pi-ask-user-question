/** Model-visible contract copied from Dano commit 288171151096364d408e97f388e0bae683590a02. */
export const pinnedDanoContract = {
  propertyNames: [
    "question", "title", "label", "prompt", "options", "choices", "inputType", "type", "input_type",
    "component", "fieldAssist", "dateFormat", "dataSource", "data_source", "multiple", "multi",
    "multipleSelect", "required", "default", "defaultValue", "prefill", "value", "confirm", "formIds", "questions",
  ],
  propertyMetadata: {
    question: { description: "Single-question call: the clear, specific question to ask the user. With questions[], top-level question/title/label/prompt is treated only as optional form instruction text; each actual field question must be inside questions[]." },
    title: { description: "Canonical grouped-form title. If omitted or malformed with questions, Dano uses the configured product default title." },
    options: { description: "Canonical choices array for this question. Dano also accepts a one-level JSON-stringified array and the choices alias. Strings remain supported; objects use stable id plus label. Include '其他' or 'Other' to let the user enter one custom answer. Omit for free-text, confirmation, or remote dataSource input." },
    fieldAssist: { description: "Controls whether text fields show Field Assist generation and polishing actions. Single-line text defaults to false; textarea defaults to true. Enable it when drafting or polishing business text would help; factual short values usually omit it." },
    dateFormat: { description: "Required when inputType is \"date\". A frontend date-control format such as \"yyyy-MM-dd\" or \"yyyy-MM-dd HH:mm\"." },
    multiple: { default: false, description: "Set true with options to allow multiple selections." },
    required: { description: "Set true to require a non-empty answer. Defaults to false." },
    confirm: { description: "Confirm one or more previously submitted grouped forms. Use {confirm:true,formIds:[\"<formId>\"]}; Dano supplies each selected form's title and latest submitted answers." },
    formIds: { description: "Standard grouped-form confirmation target: an array of formId strings returned by earlier grouped form submissions in this Assistant Turn." },
    questions: { description: "Preferred for collecting more than one answer. Make exactly one ask_user_question call with questions: [{ id, question, default, options?, multiple?, inputType?, fieldAssist?, dateFormat?, required?, dataSource? }, ...]. Every canonical non-confirmation questions[] item should include a context-based, non-empty default. A single question object or one-level JSON-stringified object/array is also accepted and normalized to an array. If title is omitted or malformed, Dano uses the configured product default. When questions is present, put each field's options, inputType, fieldAssist, dateFormat, required, dataSource, multiple, and default inside its questions[] item. Do not include top-level confirm or top-level field configuration with questions." },
  },
...
Failures use one JSON result shape: {"status":"invalid","error":{"code":"...","category":"...","message":"...","retryable":true,"issues":[{"code":"...","path":"questions[0].id","message":"..."}]}}. Correct every reported issue path in one replacement call only when retryable is true. Never retry terminal or cancelled failures.`,
  promptSnippet: "Ask the user one native question card; for several fields use one questions array with one submit button",
  promptGuidelines: [
